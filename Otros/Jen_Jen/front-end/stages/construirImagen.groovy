def generarPreinstall() {
    ...
    dir('docker_npm_preinstall') {
        def content = "FROM markadams/chromium-xvfb-js:9\n"
        ...
        if (conteoImagenes == "0"){
            sh "docker build -t ${PROJECT_NAME}/npm_preinstall:angular_cli ."
        }
    }
}
...
def generarNpminstall(Boolean moduloAngular = false) {
    ...
    dir ('docker_npm_install'){
        ...
        if (env.PROJECT_NAME == "fe-empresasBase" || env.PROJECT_NAME == "fe-empresasbase" ){
          content += "ENV NODE_OPTIONS=--max_old_space_size=2560\n"
        }
        if (moduloAngular){
