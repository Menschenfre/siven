...
def obtenerSpecJson(Object login, Object common){
  ...
  sh ''' set +x
    ...
    if [ -f rm ${WORKSPACE}/spec.json ] ; then
      rm ${WORKSPACE}/spec.json
    fi
    ...
    /usr/local/bin/kubectl -n ${!NAMESPACE} exec ${podName} /usr/bin/curl http://localhost:${PORT_NUMBER}${rutaSpec} > ${WORKSPACE}/spec.json
  '''
  if (!validacionSpec()){
...
def obtenerTlsProfile(String space){
    ...
    switch(space){
      case "INT":
        tlsprofile = 'ingress-TLS-integracion'
      break
      case "CERT":
